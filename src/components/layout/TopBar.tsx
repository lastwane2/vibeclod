              ...
              >
                {session.user.image ? (
                  <img
                    ...
                    alt=""
                    className="h-7 w-7 rounded-full ring-2 ring-[#E8E0D4]"
                  />
                ) : (
                  <div className="h-7 w-7 rounded-full bg-[#E8A445] flex items-center justify-center text-white text-xs font-bold">
                    {session.user.name?.[0] ?? "?"}
                  </div>
                )}
              ...
              {menuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 z-20 w-48 rounded-xl bg-white border border-[#E8E0D4] shadow-lg py-1 animate-fade-in">
                    <Link
                      href="/dashboard"
                      className="block px-4 py-2.5 text-sm text-[#2D2016] hover:bg-[#FAF6F0] transition-colors"
                      onClick={() => setMenuOpen(false)}
                    >
                      🗺️ My Path
                    </Link>
                    ...
                    <button
                      ...
                      className="block w-full text-left px-4 py-2.5 text-sm text-[#B8553A] hover:bg-[#FFF5F5] transition-colors"
                    >
                      Sign out
                    </button>
                  </div>
